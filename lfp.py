...
def plot_lfps(lfp_paths: List[Path], mouse: str, aligners: List[Rsync_aligner]) -> None:
    aligner_reactivation = aligners[-1]
    lfp_cache_path = Path(f"lfp_cache_{mouse}.npy")
    if not lfp_cache_path.exists():
        lfp = load_lfp_reactivations(
            lfp_paths[0],
            chunk_start_seconds=aligner_reactivation.first_matched_time_A / 2500,
            chunk_end_seconds=aligner_reactivation.last_matched_time_A / 2500,
        )
