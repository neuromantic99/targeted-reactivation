...
class Rsync_aligner:
    def __init__(
        ...
        if units_A == "auto" or units_B == "auto":
            ...
            good_intervals_A = raw_intervals_A[
                raw_intervals_A < 3 * np.median(raw_intervals_A)
            ]
            good_intervals_B = raw_intervals_B[
                raw_intervals_B < 3 * np.median(raw_intervals_B)
            ]
        ...
        intervals_A = (
            np.diff(pulse_times_A) * units_A
        )  # Inter-pulse intervals for sequence A
        intervals_B = (
            np.diff(pulse_times_B) * units_B
        )  # Inter-pulse intervals for sequence B
        ...
        chunk_starts_B = np.zeros(
            chunk_starts_A.shape, int
        )  # Start indicies of corresponding chunks in B.
        chunk_min_mse = np.zeros(
            chunk_starts_A.shape
        )  # Mean squared error for each chunks best alignment.
        chunk_2nd_mse = np.zeros(
            chunk_starts_A.shape
        )  # Mean sqared error for each chunks 2nd best alignment.
        ...
        chunk_mse[chunk_mse == 0] = np.min(
            chunk_mse[chunk_mse != 0]
        ...
        valid_matches = gmm.predict(log_mse) == np.argmin(
            gmm.means_
        )  # True for chunks which are valid matches.
        # Make arrays of corresponding times.
        cor_times_A = np.full(
            pulse_times_B.shape, np.nan
        )  # A pulse times corresponding to each B pulse.
        cor_times_B = np.full(
            pulse_times_A.shape, np.nan
        )  # B pulse times corresponding to each A pulse.
        for csA, csB, valid in zip(chunk_starts_A, chunk_starts_B, valid_matches):
            if valid:
                cor_times_A[csB : csB + chunk_size] = pulse_times_A[
                    csA : csA + chunk_size
                ]
                cor_times_B[csA : csA + chunk_size] = pulse_times_B[
                    csB : csB + chunk_size
                ]
    ...
    def A_to_B(self, times_A, extrapolate=True):
        ...
        """
        times_B = np.interp(
            times_A, self.pulse_times_A, self.cor_times_B, left=np.nan, right=np.nan
        )
        if extrapolate:
            pf = (
                times_A < self.first_matched_time_A
            )  # Mask indicating times pre first matched pulse.
            times_B[pf] = (
                times_A[pf] - self.first_matched_time_A
            ) / self.dAdB + self.first_matched_time_B
            pl = (
                times_A > self.last_matched_time_A
            )  # Mask indicating times post last matched pulse.
            times_B[pl] = (
                times_A[pl] - self.last_matched_time_A
            ) / self.dAdB + self.last_matched_time_B
        return times_B
    ...
    def B_to_A(self, times_B, extrapolate=True):
        ...
        """
        times_A = np.interp(
            times_B, self.pulse_times_B, self.cor_times_A, left=np.nan, right=np.nan
        )
        if extrapolate:
            pf = (
                times_B < self.first_matched_time_B
            )  # Mask indicating times pre first matched pulse.
            times_A[pf] = (
                times_B[pf] - self.first_matched_time_B
            ) * self.dAdB + self.first_matched_time_A
            pl = (
                times_B > self.last_matched_time_B
            )  # Mask indicating times post last matched pulse.
            times_A[pl] = (
                times_B[pl] - self.last_matched_time_B
            ) * self.dAdB + self.last_matched_time_A
        return times_A
...
def simulate_pulses(
    n_pulse=1000, interval=[100, 1900], units_B=2, noise_SD=2, missing_pulses=False
):
    ...
    pulse_times_B = units_B * (
        pulse_times_A + np.cumsum(np.random.normal(scale=noise_SD, size=n_pulse))
    )
    if missing_pulses:
