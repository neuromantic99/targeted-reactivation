...
class Session:
    ...
    def __init__(self, file_path, time_unit="second"):
        ...
        if os.path.splitext(file_path)[1] == ".txt":
            ...
            self.task_name = next(
                line for line in info_lines if "Task name" in line
            ).split(" : ")[1]
            self.subject_ID = next(
                line for line in info_lines if "Subject ID" in line
            ).split(" : ")[1]
            datetime_string = next(
                line for line in info_lines if "Start date" in line
            ).split(" : ")[1]
            ...
            self.variables_df.columns = pd.MultiIndex.from_arrays(
                [["values"] * len(columns), columns]
            )
            ...
            if time_unit == "ms":
                df = df.loc[
                    df["type"] != "warning", :
                ]  # Warning rows have nan time so can't convert to int.
            ...
            self.task_name = df.loc[
                (df["type"] == "info") & (df["subtype"] == "task_name"), "content"
            ].item()
            self.subject_ID = df.loc[
                (df["type"] == "info") & (df["subtype"] == "subject_id"), "content"
            ].item()
            datetime_string = df.loc[
                (df["type"] == "info") & (df["subtype"] == "start_time"), "content"
            ].item()
            ...
            df.loc[df["type"] == "variable", "content"] = df.loc[
                df["type"] == "variable", "content"
            ].apply(json.loads)

            self.variables_df = pd.DataFrame(
                df.loc[df["type"] == "variable", "content"].tolist()
            )
            columns = self.variables_df.columns
            self.variables_df.columns = pd.MultiIndex.from_arrays(
                [["values"] * len(columns), columns]
            )
            self.variables_df.insert(
                0, "subtype", df.loc[df["type"] == "variable", "subtype"].tolist()
            )
            self.variables_df.insert(
                0, "time", df.loc[df["type"] == "variable", "time"].tolist()
            )
            self.variables_df.reset_index()
...
class Experiment:
    def __init__(self, folder_path, time_unit="second"):
        ...
        new_files = [
            f for f in files if f.endswith((".txt", ".tsv")) and f not in old_files
        ]
            ...
            for file_name in new_files:
                try:
                    self.sessions.append(
                        Session(os.path.join(self.path, file_name), time_unit)
                    )
    ...
    def get_sessions(self, subject_IDs="all", when="all"):
        ...
        if when == "all":  # Select all sessions.
            ...
            if ... in when:  # Select a range..
                if len(when) == 3:  # Start and end points defined.
                    assert type(when[0]) == type(
                        when[2]
                    ), "Start and end of time range must be same type."
                    if type(when[0]) == int:  # .. range of session numbers.
                        when_func = lambda session: when[0] <= session.number <= when[2]
                    else:  # .. range of dates.
                        when_func = (
                            lambda session: _toDate(when[0])
                    ...
                    if type(when[1]) == int:  # .. range of session numbers.
                        ...
                        when_func = lambda session: session.datetime.date() <= _toDate(
                            when[1]
                        )
                    ...
                    if type(when[0]) == int:  # .. range of session numbers.
                        ...
                        when_func = (
                            lambda session: _toDate(when[0]) <= session.datetime.date()
                        )

            else:  # Select specified..
                assert all(
                    [type(when[0]) == type(w) for w in when]
                ), "All elements of 'when' must be same type."
        ...
        valid_sessions = [
            s for s in self.sessions if s.subject_ID in subject_IDs and when_func(s)
        ]
...
def _toDate(d):  # Convert input to datetime.date object.
    if type(d) is str:
        try:
            ...
            raise ValueError(
                "Unable to convert string to date, format must be YYYY-MM-DD."
            )
...
def session_dataframe(
    file_path, paired_events={}, pair_end_suffix=None, time_unit="second"
):
    ...
    if filetype == ".txt":  # Load data from .txt. file.
        ...
        for line in all_lines:
            if line[0] == "I":  # Info line.
                ...
                line_dicts.append(
                    {"time": 0, "type": "info", "subtype": name, "content": value}
                )
                ...
                try:  # print_variables output.
                    ...
                    line_dicts.append(
                        {
                            "time": (
                                timestamp if time_unit == "ms" else timestamp / 1000
                            ),
                            "type": "variable",
                            "subtype": "print",
                            "content": value_dict,
    ...
    # Add state durations.
    df.loc[df["type"] == "state", "duration"] = -df.loc[
        df["type"] == "state", "time"
    ].diff(-1)
    ...
    if pair_end_suffix:
        event_names = (
            event_IDs.keys()
            if filetype == ".txt"
            else set(df.loc[df.type == "event", "content"])
        )
        ...
        for end_event in end_events:
            ...
            try:
                start_event = next(
                    ev for ev in event_names if ev.startswith(stem) and ev != end_event
                )
        ...
        for i in df.index:
            ...
            if df.loc[i, "content"] in paired_events.keys():  # Pair start event.
                ...
                if start_times[start_event] is not None:
                    df.loc[start_inds[start_event], "duration"] = (
                        df.loc[i, "time"] - start_times[start_event]
                    )
...
def experiment_dataframe(
    folder_path, paired_events={}, pair_end_suffix=None, time_unit="second"
):
    ...
    assert time_unit in ("second", "ms"), 'time_unit must be "second" or "ms"'
    session_filenames = [
        f for f in os.listdir(folder_path) if f.endswith((".txt", ".tsv"))
    ]
    session_dataframes = []
